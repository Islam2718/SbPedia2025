...
export default defineComponent({
  ...
  methods: {
    async handleSubmit() {
      ...
      try {
        if (this.pageId) {
          const response = await (axios as any).put('https://api.aaatradeinternational.com/api/company/' + this.pageId,
            this.formData, 
            {
              headers: {
          ...
          const response = await (axios as any).post('https://api.aaatradeinternational.com/api/company', this.formData,
          {
            headers: {
              Authorization: `Bearer ${this.token}`,
            }
    ...
    async fetchData(id: number) {
      try {
        ...
        const response = await (axios as any).get(`https://api.aaatradeinternational.com/api/company/${id}`);
