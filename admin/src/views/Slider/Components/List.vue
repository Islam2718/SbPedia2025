  ...
  <div class="table-responsive">
    <table class="table card-table">
      ...
      <tbody class="table-border-bottom-0">
        <tr v-for="slider in sliderData" :key="slider.id">
          <td>{{ slider.id }}</td>
          <td>{{ slider.title.substring(0,50) }}</td>
          <td>{{ slider.link_url}}</td>
          <td>{{ slider.description}}</td>
          <td>
            <div class="form-check form-switch">
              <input class="form-check-input" type="checkbox" role="switch" :id="'status' + slider.id" :checked="slider.status == 1 ? true : false" @click="changeStatus(slider.id)">
            </div>
          </td>
          <td class="text-center">
            <router-link 
              :to="{ name: 'Slider', params: { id: slider.id.toString() } }"
              class="btn btn-sm btn-warning me-2"
            >Edit</router-link>
            <button type="button" class="me-2 btn btn-sm btn-danger" 
              data-bs-toggle="modal" data-bs-target="#modalCenter"
              @click="fetchItem(slider)"
            >
              Delete
            </button>
          </td>
...
export default defineComponent({
  ...
  data() {
    ...
    return {
      ...
      sliderData: [] as Slider[], // Explicitly define the type as Post[]
      deletevalue: {
        id: 0,
        title: "",
        name: "",	
  ...
  methods: {
    async fetchAllData() {
      try {
        const response = await fetch('https://api.aaatradeinternational.com/api/slider');

        if (response.status === 200) {
          const data = await response.json();
          this.sliderData = data.data;
          console.log('_sliderData_', this.sliderData);
        }
      } catch (error) {
        console.error(error);
      }
    ...
    async changeStatus(id: number) {
      try {
        const response = await (axios as any).get('https://api.aaatradeinternational.com/api/slider/'+id+'/status', {
          headers: {
    ...
    async deleteItem(id: number) {
      try {
        const response = await (axios as any).delete('https://api.aaatradeinternational.com/api/slider/'+id, {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
        });
