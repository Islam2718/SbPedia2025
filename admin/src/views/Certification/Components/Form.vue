  ...
  methods: {
      ...
      try {
        if (this.pageId) {
          const response = await (axios as any).put(
            'https://api.aaatradeinternational.com/api/certification/' + this.pageId,
            this.formData, 
            {
              headers: {
          ...
          const response = await (axios as any).post('https://api.aaatradeinternational.com/api/certification', this.formData, {
            headers: {
              Authorization: `Bearer ${this.token}`,
            }
          });
    ...
    async fetchData(id: number) {
      try {
        ...
        const response = await (axios as any).get(`https://api.aaatradeinternational.com/api/certification/${id}`, {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
        });
