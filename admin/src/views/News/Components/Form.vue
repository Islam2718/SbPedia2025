...
export default defineComponent({
  ...
  name: "newsForm", // Rename the component to a multi-word name
  data() {
  ...
  methods: {
      ...
      try {
        if (this.pageId) {
          const response = await (axios as any).put(
            'https://api.aaatradeinternational.com/api/news/' + this.pageId,
            this.formData, 
            {
              headers: {
          ...
          const response = await (axios as any).post('https://api.aaatradeinternational.com/api/news', this.formData, {
            headers: {
              Authorization: `Bearer ${this.token}`,
            }
          });
    ...
    async fetchData(id: number) {
      try {
        ...
        const response = await (axios as any).get(`https://api.aaatradeinternational.com/api/news/${id}`, {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
        });
        console.log('_news_data_by_ID_', response);
