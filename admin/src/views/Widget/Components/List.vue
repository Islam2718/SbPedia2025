  ...
  <div class="table-responsive">
    <table class="table card-table">
      ...
      <tbody class="table-border-bottom-0">
        <tr v-for="widget in widgetData" :key="widget.id">
          <td>{{ widget.id }}</td>
          <td>{{ widget.title}}</td>
          ...
          <td>
            <div class="form-check form-switch">
              <input class="form-check-input" type="checkbox" role="switch" :id="'flexSwitchCheckChecked' + widget.id" :checked=" widget.status == 1 ? true : false" @click="changeStatus(widget.id)">
            </div>
          </td> 
          <td class="text-center">
            <router-link 
              :to="{ name: 'Widget', params: { id: widget.id.toString() } }"
              class="btn btn-sm btn-warning me-2"
            >Edit</router-link>
            <button type="button" class="me-2 btn btn-sm btn-danger" 
              data-bs-toggle="modal" data-bs-target="#modalCenter"
              @click="fetchItem(widget)"
            >
              Delete
            </button>
          </td>
...
export default defineComponent({
  ...
  data() {
    ...
    return {
      ...
      widgetData: [] as Widget[], // Explicitly define the type as Post[]
      deletevalue: {
        id: 0,
        title: "",
        name: "",	
  ...
  methods: {
    async fetchAllData() {
      try {
        const response = await (axios as any).get('https://api.aaatradeinternational.com/api/widget');

        if (response.status === 200) {
          // const data = await response.json();
          const data = await response.data.data;
          this.widgetData = data;
          console.log('_widgetData_', this.widgetData);
        }
      } catch (error) {
        console.error(error);
      }
    ...
    async deleteItem(id: number) {
      try {
        ...
        console.log('del_widget', response); 
        if (response.status == 200) {
          this.fetchAllData();
          this.successStatus = 1
          setTimeout(() => {
