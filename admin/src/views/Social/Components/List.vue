  ...
  <div class="table-responsive">
    <table class="table card-table">
      ...
      <tbody class="table-border-bottom-0">
        <tr v-for="social in socialData" :key="social.id">
          <td>{{ social.id }}</td>
          <td>{{ social.name }}</td>
          <td>{{ social.icon }}</td>
          <td>{{ social.link_url }}</td>
          <td>
            <div class="form-check form-switch">
              <input class="form-check-input" type="checkbox" role="switch" :id="'flexSwitchCheckChecked' + social.id" :checked="social.status == 1 ? true : false " @click="changeStatus(social.id)">
            </div>
          </td>
          <td class="text-center">
            <router-link 
              :to="{ name: 'Social', params: { id: social.id.toString() } }"
              class="btn btn-sm btn-warning me-2"
            >Edit</router-link>
            <button type="button" class="me-2 btn btn-sm btn-danger" 
              data-bs-toggle="modal" data-bs-target="#modalCenter"
              @click="fetchItem(social)"
            >
              Delete
            </button>
          </td>
...
export default defineComponent({
  ...
  data() {
    ...
    return {
      ...
      socialData: [] as Social[], // Explicitly define the type as Post[]
      deletevalue: {
        id: 0,
        title: "",
        name: "",	
  ...
  methods: {
    async fetchAllData() {
      try {
        const response = await fetch('https://api.aaatradeinternational.com/api/social');

        if (response.status === 200) {
          const data = await response.json();
          this.socialData = data.data;
          console.log('_socialData_', this.socialData);
        }
      } catch (error) {
        console.error(error);
      }
...
//
    async changeStatus(id: number) {
      try {
        const response = await (axios as any).get('https://api.aaatradeinternational.com/api/social/'+id+'/status', {
          headers: {
    ...
    async deleteItem(id: number) {
      try {
        const response = await (axios as any).delete('https://api.aaatradeinternational.com/api/social/'+id, {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
        });
