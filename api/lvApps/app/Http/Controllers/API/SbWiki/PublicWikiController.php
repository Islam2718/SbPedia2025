...
use App\Models\SbWiki\WikiContent;
use App\Models\SbWiki\WikiContentLanguage;
...
class PublicWikiController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $query = WikiContent::with(['wiki_content_language' => function ($query) use ($lang) {
            if ($lang) {
                $query->where('setting_language_id', $lang);
            }
        }]);
        ...
        $filteredData = $response->filter(function ($item) {
            return $item->wiki_content_language->isNotEmpty();
        });
